...
> Stage directions are denoted `like this`. If you want a style reference, think of something very similar to 3Blue1Brown. I'll even be using Manim for the animations.

`A one-second clip of gameplay plays.`

Pause.

`The clip freezes mid-motion.`

When you press "save," you're doing something quietly extraordinary. You're freezing an entire universe in place -- every object, every variable, every unfinished intention -- perfectly suspended.

`The frozen frame fractures into clusters of shards, thin filaments linking one cluster to another.`

And when you load that save, you're not simply returning to the past. You're replacing the present with a version that has no memory of what came after.

`A few clusters vanish. The remaining shards slide into the shape of an earlier moment.`

But what happens when the world remembers?

`Camera pulls back, framing the moment inside a node. The erased clusters reappear with faint, translucent threads connecting them.`

What if characters, systems, even the story itself responded not just to what you chose -- but to the fact that you chose *again?*

`Fade to black. The project's title writes itself onto the screen, letter by letter.`

That question is the foundation of the system I, Jenson Searle, have been building -- a system where saving and loading aren't external tools for player convenience, but laws of physics. In this system, the timeline isn't a straight line. It's a web woven out of your revisions. Each save stitches a new thread into that fabric.

`A single thread extends from the final letter. Camera zooms in along the thread, letting the title drift out of frame.`

To understand why this matters, we need to look underneath.

`Three node-clusters appear along the growing thread.`

Most save systems flatten time. They take the entire world -- the player, the state of quests, variables scattered across the map --

`A player character, a quest log, and bits of the environment appear, shrinking until they're absorbed into the final node.`

-- and serialize it into a snapshot. When you load, that snapshot overwrites the present. It's efficient, reliable, perfectly usable for most games... and a total amnesia event for the world.

`The two earlier nodes fade out cleanly.`

`The faded nodes reappear.`

`Camera rotates, revealing a much larger 3D network of nodes hidden behind them.`

This system works differently. It records not only how the world *is*, but *how it became that way.*

`Edges and nodes begin to glow, starting from an unseen root.`

Every save becomes a node in a branching structure -- a graph of cause and consequence.

`A wave of pulses travels through the graph.`

It captures what led to the moment, and what you chose to undo.

`Camera zooms into a dense subgraph.`

Now the world can ask:
- "How many times has this moment been visited?"
- "Which paths converge on the same decision?"
- "What keeps pulling the player back to this branch?"

`Threads sprout and split as the graph grows, forming new nodes.`

`One node is highlighted.`

`A small counter appears: "3 visits".`

For designers, this opens up a layer of interactivity that didn't exist before. You can query not just the state, but the player's intent.

`The word "Betrayal?" appears under the counter.`

Maybe an NPC starts to notice you always return to them before something goes wrong.

`Another thread connects to a puzzle node. "Puzzle Changed!" appears.`

Maybe a puzzle restructures itself because you've solved it twice already.

`Saturation drains from the graph as the screen fades to black.`

Maybe the timeline itself starts to strain under too many revisions.

`Fade in on a single node.`

`Label: "Current Save". A new child node sprouts from it. The label transfers to the new node.`

Technically, it works because every save produces a *new* node -- never a replacement. Nodes carry metadata: their parent, their time of creation, even whether the save was created quietly by the system.

`The root node lights up. The player loads an older save. A new child node appears, linking to both prior nodes, along with the label moving to this new third node.`

The result is a causal map of the player's journey -- a record of how they think. Designers can query this graph:
- "How many times has this object been restored?"
- "Which saves share this piece of state?"

`A wide view reveals a sprawling graph with multiple converging and diverging threads.`

The innovation isn't persistence. It's context.

`Zoom into a leaf node. Gameplay footage fills the node. Continue zoom until the footage fully replaces the view.`

For players, the effect is subtle but meaningful. Saving stops being a utility function and becomes a choice the world can react to. Instead of the world resetting politely around you, the game begins to acknowledge your habits. If you always reload before a bad outcome, the world starts to treat that pattern as part of who you are.

In the prototype, this takes the form of the Gallery: a physical space where each save manifests as a painting. Re-entering a painting means re-entering that moment, and the world acknowledges it.

Paintings decay over resets. Dialogue shifts. Systems detect patterns. Puzzles begin to depend on your personal history with them.

The world doesn't let you rewrite history, but it remembers that you tried.

`Zoom back out until the gameplay footage sits frame inside its node. That frame dissolves. The camera continues pulling back.`

Most save systems are invisible. This one is meant to be seen -- by the player, and by the world.

And when you step into a world that remembers you, every choice becomes a mark the game carries forward. It's a way of letting players -- and designers -- explore stories that can only exist when memory itself becomes part of play.

If *every* action you take becomes part of the story...

`Zoom out to reveal the graph has arranged itself into the shape of the project logo.`

... the line between memory and consequence starts to blur.
